...
class M_continous extends CI_Model {
    private $db;
    ...
    {
        $queryPeriode = $this->db->query("SELECT DISTINCT(PERIODE) as periode FROM CIF order by periode ASC")->result();
        return $queryPeriode;
    }

    public function periodenik()
    {
        $queryPeriode = $this->db->query("SELECT DISTINCT(PERIODE) as periode FROM CIF Where periode='20220610'")->result();
        return $queryPeriode;
    }

    public function getdata($mulaiHalaman,$akhirHalaman)
    // public function getdata($mulaiHalaman,$akhirHalaman,$cari)
    {
        $dataContinous  = $this->db->query("SELECT * FROM (
                        SELECT ROW_NUMBER() OVER(order by FTGLINPUT DESC) as no
                            ,FCODE AS KODE_CIF
                            ,FKDCAB
                            ,FNAMA
                            ,REPLACE(FNAMAIBU, '','KOSONG') AS FNAMAIBU
                            ,FTGLINPUT
                            ,FNPWP
                            ,FNOMORID
                            ,FSTATUS
                            ...
                            (
                                LEN(FNAMAIBU) = 1 
                                OR LEN(FNAMAIBU) = 2 
                                OR LEN(FNAMAIBU) = NULL 
                                OR LEN(FNAMAIBU) = ''
		                        -- OR REPLACE(REPLACE(PERIODE,' ',''),'-','') like '%'+REPLACE(REPLACE(RTRIM('$cari'),' ',''),'-','')+'%'
                            )
                        ) as a
                        WHERE no between $mulaiHalaman and $akhirHalaman ORDER BY no ASC ")->result();

        $jumlah = $this->db->query("SELECT SUM(jml) AS jml  FROM (
                        SELECT count(*) as jml
                        FROM TBPENGIRIM
                        WHERE
                            LEN(FNAMAIBU) = 1 
    ...
    public function Anomaliibu($mulaiHalaman,$akhirHalaman,$cari,$kd_cabang,$cek_tgl_input,$cek_tgl_inputakhir,$cek_periode)
    {
        $dataIbu  = $this->db->query("SELECT * FROM (
                                        SELECT ROW_NUMBER() OVER(order by PERIODE DESC) as NO
                                            ...
                                            ,CONVERT(Varchar(20),[FTGLINPUT],103) as [TGL_INPUT]
                                        ...
                                        and PERIODE = '$cek_periode'
                                        and CIF.[FTGLINPUT] between '$cek_tgl_input' and '$cek_tgl_inputakhir'
                                        -- and [FKDCAB] in (001,002)
                                        and [FKDCAB] = '$kd_cabang'
                                        and [FNAMA] like '%$cari%'
        ...
        $jumlah = $this->db->query("SELECT SUM(jml) AS jml  FROM (
                                        ...
                                        WHERE 
                                            ...
                                            and PERIODE = '$cek_periode'
                                            and FTGLINPUT between '$cek_tgl_input' and '$cek_tgl_inputakhir'
                                            -- and FKDCAB in (001,002)
                                            and FKDCAB = '$kd_cabang'
                                            and FNAMA like '%$cari%'
                                            -- order by FTGLINPUT desc
                                            ) as a")->row()->jml;
    ...
    public function getDataNik($mulaiHalaman,$akhirHalaman,$cari,$kd_cabang,$cek_tgl_input,$cek_tgl_inputakhir,$cek_periode)
    {
        $dataNsbhIndividu = $this->db->query("SELECT * FROM (
                                        SELECT ROW_NUMBER() OVER(order by PERIODE DESC) as NO
                                            ,CASE WHEN [FKDCAB] = 000 then 'KANTOR PUSAT'
                                                WHEN [FKDCAB] = 999 then 'KANTOR PUSAT'
                                                WHEN [FKDCAB] = 001 then 'KC JATINEGARA'
        ...
        $jumlahNsbhIndividu = $this->db->query("SELECT SUM(jml) AS jml  FROM (
                                    SELECT count(*) as jml
                                    FROM CIF
                                    WHERE 
                                        FJNSNSB = 'I' 
        ...
        return [
            'data' => $dataNsbhIndividu,
            'jumlah'=> $jumlahNsbhIndividu
        ];
    }

    public function getDataNpwp($mulaiHalaman,$akhirHalaman,$cari,$kd_cabang,$cek_tgl_input,$cek_tgl_inputakhir,$cek_periode)
    {
        $dataNpwp = $this->db->query("SELECT * FROM (
                                        ...
                                        WHERE CIF.FJNSNSB = 'I' 
                                        and [FSTATUS] = 1
                                        -- and ([FNPWP]  not like '%__.___.___._.___.000%'
                                        -- and ([FNPWP]  not like '[0-9][0-9].___.___._.___.___'
                                            -- or [FNPWP] = ''
                                            -- or len ([FNPWP]) = 20
                                            -- or ltrim(rtrim(FNPWP)) <> '99'
                                            -- )
        ...
        $jumlahNpwp = $this->db->query("SELECT SUM(jml) AS jml  FROM (
                                    SELECT count(*) as jml
                                    FROM CIF
                                    WHERE 
                                        FJNSNSB = 'I' 
                                        ...
                                        -- and ([FNPWP]  not like '[0-9][0-9].___.___._.___.___'
                                            -- or [FNPWP] = ''
                                            -- or len ([FNPWP]) = 20
                                            -- or [FNPWP] <> '99') //coba tutup dulu 19-10-2022
                                            -- or ltrim(rtrim(FNPWP)) <> '99'
                                            -- )
                                        and [FNPWP] not like '__.___.___._.___.___%'
                                        and FNPWP not in ('99')
    ...
    public function getDataUmum($mulaiHalaman,$akhirHalaman,$cari,$kd_cabang,$cek_tgl_input,$cek_tgl_inputakhir,$cek_periode)
    {
        ...
        $jumlahUmum = $this->db->query("SELECT SUM(jml) AS jml  FROM (
                                    SELECT count(*) as jml
                                    FROM CIF
                                    WHERE 
                                        FJNSNSB = 'I' 
    ...
    public function getDataNik_GakPAKE($mulaiHalaman,$akhirHalaman,$cari,$kd_cabang,$cek_tgl_input,$cek_tgl_inputakhir,$cek_periode)
    {
        ...
        $jumlahNik = $this->db->query("SELECT SUM(jml) AS jml  FROM (
                                    SELECT count(*) as jml
                                    FROM CIF
                                    WHERE 
                                        FJNSNSB = 'I' 
    ...
    public function detailAll($nomor_cif)
    {
        $detailAll = $this->db->query("SELECT ROW_NUMBER() OVER(order by PERIODE DESC) as NO
                                    ,CASE WHEN [FKDCAB] = 000 then 'KANTOR PUSAT'
                                        WHEN [FKDCAB] = 999 then 'KANTOR PUSAT'
                                        WHEN [FKDCAB] = 001 then 'KC JATINEGARA'
                                        WHEN [FKDCAB] = 002 then 'KC MANGGA DUA'
